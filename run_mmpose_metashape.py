...
# run mmpose
cmd = 'python demo/topdown_demo_with_mmdet.py demo/mmdetection_cfg/rtmdet_m_640-8xb32_coco-person.py rtmdet_m_8xb32-100e_coco-obj365-person-235e8209.pth configs/wholebody_2d_keypoint/rtmpose/ubody/rtmpose-l_8xb32-270e_coco-ubody-wholebody-384x288.py dw-ll_ucoco_384.pth  --input ' + osp.join(dataset_path, 'person', 'ldr_undistorted') + ' --output-root ' + output_root + ' --save-predictions'
print(cmd)
os.system(cmd)

# move predictions to the root path
os.makedirs(osp.join(dataset_path, 'keypoints_whole_body'), exist_ok=True)
img_path_list = sorted(glob(osp.join(dataset_path, 'person', 'ldr_undistorted', '*.png')) + glob(osp.join(dataset_path, 'person', 'ldr_undistorted', '*.jpg')))
...
video_save = cv2.VideoWriter(osp.join(dataset_path, 'keypoints_whole_body.mp4'), cv2.VideoWriter_fourcc(*'mp4v'), 30, (video_width*2, video_height)) 
for i, output_path in enumerate(output_path_list):
    ...
    output = cv2.resize(output, (video_width, video_height))
